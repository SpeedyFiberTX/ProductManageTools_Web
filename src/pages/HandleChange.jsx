...
export default function HandleChange() {
  ...
  const handleSubmitAll = async () => {
    ...
    if (!confirm(`即將為 ${payload.length} 筆資料進行 Handle 變更，是否確認？`)) {
      return;
    }

    setIsSubmitting(true);
    try {
      await postJson("/api/handleChangeUpdater", { rows: payload });
      alert("Handle 變更請求已成功送出。");
    } catch (error) {
  ...
  return (
    <>
      <Hero
        pageTitle="批次變更 Handle"
        onSubmit={handleSubmitAll}
        isSubmitting={isSubmitting}
        submitButtonText="開始變更"
      />
                  ...
                  <table className="w-full text-sm text-left text-slate-500">
                    <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                      <tr>
                        ...
                        <th scope="col" className="px-6 py-3">Old Handle</th>
                      </tr>
                    </thead>
                    <tbody>
                      {previewData.map((data, index) => (
                        <tr key={index} className="bg-white border-b hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4 font-mono text-slate-800">{data.oldHandle}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
